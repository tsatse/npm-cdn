...
Package.prototype.download = function(callback) {
  npm.load({}, function() {
    ...
    if(fs.existsSync(path.join(npmPrefix, 'lib/node_modules', this.name))) {
      ...
      console.log('creating dir ', path.resolve(cachedir, this.name))
      fs.mkdirSync(path.resolve(cachedir, this.name))
      fs.mkdirSync(this.directory)
      fs.mkdirSync(path.resolve(cachedir, this.name, "package"))
